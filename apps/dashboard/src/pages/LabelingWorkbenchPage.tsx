...
import { PenTool, SkipForward, Flag, Send } from "lucide-react";
...
export function LabelingWorkbenchPage() {
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.functions.invoke("submit-annotation", {
      body: {
        task_id: task.id,
        labels,
  ...
  if (!task) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <PenTool className="mb-4 h-12 w-12 text-[var(--color-muted-foreground)]" />
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Labeling Workbench</h1>
          <p className="text-[var(--color-muted-foreground)]">{task.project.name}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <pre className="whitespace-pre-wrap rounded-lg bg-[var(--color-muted)] p-4 text-sm">
